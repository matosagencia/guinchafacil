...
function ensureGuincho(array $usuario, string $placa): array
{
    ...
    try {
        $pdo->prepare('UPDATE guinchos SET reboque_aprovado = 1 WHERE id = ?')->execute([(int)$guincho['id']]);
    } catch (Throwable $e) {
        // Em bases antigas, a coluna pode não existir; o fallback fica igual ao comportamento anterior.
    }
