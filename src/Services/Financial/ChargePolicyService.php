...
final class ChargePolicyService
{
    ...
    /**
     * Cobrança adicional da oficina móvel quando o cliente aprova o orçamento.
     * O valor é da plataforma e não substitui a comissão percentual do
     * atendimento inicial nem a eventual cobrança de um reboque posterior.
     */
    public static function itemOrcamentoOficinaMovelAprovado(float $feeAmount, array $snapshot = []): array
    {
        if ($feeAmount < 0) {
            throw new \InvalidArgumentException('A comissão do orçamento aprovado não pode ser negativa.');
        }

        return [
            'phase_code' => ChargeCodes::PHASE_MOBILE_QUOTE_APPROVAL,
            'charge_type' => ChargeCodes::TYPE_MOBILE_QUOTE_APPROVED_FEE,
            'description' => 'Comissão de orçamento aprovado por oficina móvel',
            'quantity' => 1,
            'unit_amount' => round($feeAmount, 2),
            'gross_amount' => round($feeAmount, 2),
            'discount_amount' => 0,
    ...
    public static function criarCobrancaOrcamentoOficinaMovelAprovado(
        int $orderId,
        int $providerId,
        float $feeAmount,
        array $snapshot,
        ...
        require_once __DIR__ . '/../../Models/Financial/OrderChargeItem.php';
        return OrderChargeItem::criar(array_merge(
            self::itemOrcamentoOficinaMovelAprovado($feeAmount, $snapshot),
            [
                'order_id' => $orderId,
                'provider_id' => $providerId,
                'idempotency_key' => $idempotencyKey,
