...
final class ConversionService
{
    ...
    public static function decidirConversao(int $pedidoId, int $clienteId, bool $aprovado, array $destino = []): PedidoTransitionResult
    {
        ...
        $saidaOficinaComDesconto = PedidoDecisaoSocorro::possuiDecisao($pedidoId, PedidoDecisaoSocorro::SAIDA_OFICINA_COM_DESCONTO);
        $descontoSaidaOficina = 0.0;
        ...
        if ($saidaOficinaComDesconto) {
            $descontoSaidaOficina = (float)Configuracao::get('desconto_saida_oficina_percentual', 0.21);
            $descontoSaidaOficina = max(0.0, min(1.0, $descontoSaidaOficina));
            $custoReboque = round($custoReboque * (1 - $descontoSaidaOficina), 2);
        }
        ...
        try {
            ...
            // O desconto de saida substitui o credito de conversao; nao
            // acumulamos duas vantagens comerciais no mesmo pedido.
            if (!$saidaOficinaComDesconto && $creditoConversao > 0) {
