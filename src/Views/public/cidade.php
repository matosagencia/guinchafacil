...
$schemaBusiness = ['@context'=>'https://schema.org','@type'=>'AutomotiveBusiness','name'=>'GuinchaFÃ¡cil','url'=>$url,'description'=>$descricao,'areaServed'=>['@type'=>'City','name'=>$nomeCidade,'address'=>['@type'=>'PostalAddress','addressLocality'=>$nomeCidade,'addressRegion'=>$uf,'addressCountry'=>'BR']],'serviceType'=>['Guincho','AssistÃªncia veicular','Troca de pneu','Partida auxiliar','Pane seca']];
$schemaServices = ['@context'=>'https://schema.org','@type'=>'ItemList','name'=>"ServiÃ§os de assistÃªncia veicular em {$nomeCidade}",'itemListElement'=>[]];
foreach (($servicosCatalogo ?? []) as $posicao => $servico) {
    $schemaServices['itemListElement'][] = ['@type'=>'ListItem','position'=>$posicao + 1,'item'=>['@type'=>'Service','name'=>(string)($servico['name'] ?? ''),'description'=>(string)($servico['description'] ?? ''),'provider'=>['@type'=>'AutomotiveBusiness','name'=>'GuinchaFÃ¡cil','url'=>$url],'areaServed'=>['@type'=>'City','name'=>$nomeCidade]]];
}
