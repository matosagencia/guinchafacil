...
window.GFPushConfig = <?= json_encode([
    ...
    'subscribeUrl' => $bp . '/push/subscribe',
    'unsubscribeUrl' => $bp . '/push/unsubscribe',
    'csrfToken' => $csrf,
    'role' => 'especialista',
    ...
    <main class="main-content especialista-dashboard-main">
        <header class="page-head mb-4">
            <div>
                ...
                <h1>Olá, <?= $nome ?></h1>
                <p>Chamados, localização, evidências e repasses em um só lugar.</p>
            </div>
            <div class="d-flex align-items-center gap-2">
                <span class="badge <?= $online ? 'text-bg-success' : 'text-bg-secondary' ?>"><?= $online ? 'Online' : 'Offline' ?></span>
                <form method="post" action="<?= htmlspecialchars($bp) ?>/especialista/disponibilidade">
                    <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrf) ?>">
                    <input type="hidden" name="disponivel" value="<?= $online ? '0' : '1' ?>">
                    <button class="btn btn-warning"><i class="fas fa-power-off me-1"></i><?= $online ? 'Ficar offline' : 'Ficar online' ?></button>
                </form>
            </div>
        </header>
        ...
        <section class="card shadow-sm mb-4">
            <div class="card-body d-flex flex-wrap justify-content-between align-items-center gap-3">
                <div>
                    <h2 class="h6 mb-1">Alertas no navegador</h2>
                    <p class="text-muted mb-0">Receba chamados mesmo com a aba fechada. Use o navegador como canal secundário do painel.</p>
                </div>
                <div class="d-flex align-items-center gap-2">
                    <span class="badge text-bg-secondary" data-push-state>Inativo</span>
                    <button type="button" class="btn btn-outline-primary btn-sm" data-push-toggle <?= $pushPublicKey === '' ? 'title="Configure PUSH_VAPID_PUBLIC_KEY no .env"' : '' ?>>
                        Ativar alertas
                    </button>
                </div>
            </div>
        ...
        <?php endforeach; ?>
        <?php if (!$especialista): ?>
            <div class="alert alert-warning">Perfil operacional não encontrado. Contate o administrador.</div>
        <?php elseif (!$aprovado): ?>
            <div class="alert alert-info">Seu cadastro está em análise. As ferramentas serão liberadas após a homologação.</div>
        <?php endif; ?>

        <section class="especialista-hero mb-4" id="perfil">
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
                <div>
                    <span class="text-uppercase small">Status do perfil</span>
                    <h2 class="h4 mb-1"><?= $aprovado ? 'Pronto para atender' : 'Cadastro em análise' ?></h2>
                    <span class="text-muted">GPS e evidências são registrados durante cada atendimento.</span>
                </div>
                <div class="text-end">
                    <div class="especialista-stars" aria-label="<?= number_format($reputacao, 1, ',', '.') ?> de 5 estrelas">
                        <?php for ($i = 1; $i <= 5; $i++): ?>
                            <i class="fas fa-star<?= $i <= round($reputacao) ? '' : ' opacity-50' ?>"></i>
                        <?php endfor; ?>
                    </div>
                    <small><?= number_format($reputacao, 1, ',', '.') ?> · <?= $avaliacoes ?> avaliações</small>
                </div>
            </div>
        </section>

        <section id="ferramentas" class="mb-4">
            <h2 class="h5 mb-3">Ferramentas rápidas</h2>
            <div class="row g-3">
        ...
        <section id="ganhos" class="mt-4">
            <div class="card"><div class="card-body"><h2 class="h6">Resumo de ganhos e repasses</h2><p class="text-muted mb-0">Os valores ficam disponíveis após a conclusão e confirmação do pagamento do incidente.</p></div></div>
        </section>
    </main>
</div>
