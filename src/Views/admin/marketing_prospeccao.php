...
$fix = static function ($value): string {
    $text = (string)$value;
...
$utf8fix = static function ($value): string {
    $text = (string)$value;
    if ($text === '' || (strpos($text, 'Ã') === false && strpos($text, 'Â') === false)) {
        return $text;
    }
...
$localidade = static function (array $r) use ($fix, $utf8fix): string {
    $cidade = trim($utf8fix($r['cidade'] ?? ''));
    $uf = trim($utf8fix($r['uf'] ?? ''));
    $nome = trim($utf8fix($r['nome'] ?? ''));
...
#prospeccao .badge,
#prospeccao .text-bg-secondary,
#prospeccao .text-bg-dark,
#prospeccao .text-bg-success,
#prospeccao .text-bg-warning,
#prospeccao .text-bg-danger,
#prospeccao .text-bg-primary,
#prospeccao .text-bg-info {
    color: #fff !important;
}

#prospeccao .form-control,
#prospeccao .form-select,
#prospeccao textarea {
...
#prospeccao .table td,
#prospeccao .table th,
#prospeccao .table span,
#prospeccao .table strong {
    color: #17212b;
}

#prospeccao .tab-pane,
#prospeccao .tab-pane .border.rounded,
#prospeccao .tab-pane .mk-card {
    background-color: #ffffff;
    color: #17212b;
}

#prospeccao .tab-pane .text-muted,
#prospeccao .tab-pane .small,
#prospeccao .tab-pane .mk-muted {
    color: #536575 !important;
}

#prospeccao .tab-pane .form-control,
#prospeccao .tab-pane .form-select,
#prospeccao .tab-pane textarea,
#prospeccao .tab-pane .table {
    background-color: #ffffff !important;
    color: #17212b !important;
}
...
#prospeccao .nav-tabs .nav-link {
    color: #536575;
    background-color: transparent;
    border-color: transparent transparent #d5dde5;
}

#prospeccao .nav-tabs .nav-link.active {
    color: #17212b;
    background-color: #ffffff;
    border-color: #d5dde5 #d5dde5 #ffffff;
}

#prospeccao .nav-tabs .nav-link:hover,
#prospeccao .nav-tabs .nav-link:focus {
    color: #0f5132;
}

#prospeccao .form-control::placeholder,
#prospeccao textarea::placeholder {
    color: #6b7280 !important;
    opacity: 1;
}

#prospeccao .form-control:-webkit-autofill,
#prospeccao .form-control:-webkit-autofill:hover,
#prospeccao .form-control:-webkit-autofill:focus,
#prospeccao .form-select:-webkit-autofill,
#prospeccao textarea:-webkit-autofill {
...
<section class="mk-card mt-4" id="prospeccao">
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-3">
        ...
        <div class="d-flex gap-2 flex-wrap">
            ...
            <a class="btn btn-outline-dark btn-sm" href="<?= $e($bp) ?>/admin/configuracoes#ambiente"><i class="fas fa-shield-halved me-1"></i>Ver .env</a>
        </div>
    </div>
    ...
    <div class="mk-grid">
        ...
        <div class="mk-card">
            <div class="mk-muted">Oportunidades em aberto</div>
            <div class="mk-number"><?= (int)($resumo['vagas_restantes'] ?? 0) ?></div>
            <div class="mk-muted">cadastros ainda em avaliação no território</div>
        </div>
        <div class="mk-card">
                                        ...
                                        <select class="form-select" name="regiao_id" required>
                                            <?php foreach ($regioes as $r): ?>
                                                <option value="<?= (int)$r['id'] ?>">
                                                    <?= $e($fix($r['nome'])) ?> - <?= $e($localidade($r)) ?> (<?= (int)($r['quota_atingida'] ?? 0) ?>/<?= (int)($r['quota_alvo'] ?? 0) ?>)
                                                </option>
                                            <?php endforeach; ?>
                        ...
                        <div class="mk-card h-100">
                            ...
                            <div class="row g-3">
                                ...
                                <div class="col-md-6">
                                    ...
                                    <strong><?= (int)($prospeccao['vagas_restantes'] ?? 0) ?> oportunidades em aberto</strong>
                                </div>
                            </div>
                                    ...
                                    <table class="table mk-table align-middle mb-0">
                                        ...
                                        <tbody>
                                            <?php foreach ($regioes as $r): ?>
                                                <tr>
                                                    <td>
                                                        <strong><?= $e($fix($r['nome'])) ?></strong><br>
                                                        <span class="text-muted"><?= $e($localidade($r)) ?></span>
                                                    </td>
                        ...
                        <table class="table mk-table align-middle mb-0">
                            ...
                            <tbody>
                                <?php foreach ($zonasTerritoriais as $zona): ?>
                                    ...
                                    <tr>
                                        <td>
                                            <strong><?= $e($fix($zona['name'] ?? '')) ?></strong><br>
                                            <span class="text-muted small"><?= $e($fix($zona['code'] ?? '')) ?></span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div><?= $e($fix($zona['bairros_referencia'] ?? '')) ?></div>
                                            <?php if (!empty($zona['regiao_existente'])): ?>
                                                <div class="small text-muted">
                                                    <?= $e($fix($zona['regiao_existente']['nome'] ?? '')) ?> · quota <?= (int)($zona['regiao_existente']['quota_atingida'] ?? 0) ?>/<?= (int)($zona['regiao_existente']['quota_alvo'] ?? 0) ?>
                                                </div>
                                            <?php endif; ?>
                                        </td>
                                    </tr>
                    ...
                    <?php foreach ($fila as $item): ?>
                        ...
                        <div class="border rounded p-3 mb-3">
                            <div class="d-flex justify-content-between gap-3 flex-wrap">
                                <div>
                                    <strong><?= $e($fix($lead['nome_negocio'] ?? '')) ?></strong>
                                    <span class="badge text-bg-secondary ms-2"><?= $e($fix($lead['categoria'] ?? '')) ?></span>
                                    <div class="text-muted small">
                                        <?= $e($fix($regiao['nome'] ?? '')) ?> · score <?= $e((string)($lead['score_go'] ?? '0')) ?> ·
                                        <?= (int)($convite['vagas_restantes'] ?? 0) ?> oportunidade(s) em aberto
                                    </div>
                                </div>
                                <div class="text-end small text-muted">
                                    <?= $e($fix($lead['telefone'] ?? '')) ?><br>
                            ...
                            <div class="row g-3 mt-3">
                                <div class="col-lg-6">
                                    <div class="border rounded p-3 h-100">
                                        <div class="fw-semibold mb-2">Mensagem de abordagem</div>
                                        <textarea class="form-control" rows="7" readonly><?= $e($utf8fix($convite['texto'] ?? '')) ?></textarea>
                                        <div class="d-flex gap-2 mt-3 flex-wrap">
                                            <?php if (!empty($convite['wa_link'])): ?>
                                                <a href="<?= $e($convite['wa_link']) ?>" target="_blank" rel="noopener" class="btn btn-success btn-sm">
                                                    <i class="fab fa-whatsapp me-1"></i>Abrir abordagem
                                                </a>
                                            <?php endif; ?>

                                            <form method="post" action="<?= $e($bp) ?>/admin/marketing/prospeccao/lead/enviado/<?= (int)($lead['id'] ?? 0) ?>">
                                                <input type="hidden" name="csrf_token" value="<?= $e($csrfToken ?? '') ?>">
                                                <input type="hidden" name="mensagem_texto" value="<?= $e($utf8fix($convite['texto'] ?? '')) ?>">
                                                <input type="hidden" name="wa_link" value="<?= $e($utf8fix((string)($convite['wa_link'] ?? ''))) ?>">
                                                <button type="submit" class="btn btn-outline-secondary btn-sm">Enviar abordagem</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-lg-6">
                                    <div class="border rounded p-3 h-100">
                                        <div class="fw-semibold mb-2">Mensagem de como funciona</div>
                                        <textarea class="form-control" rows="7" readonly><?= $e($utf8fix($convite['texto_como_funciona'] ?? '')) ?></textarea>
                                        <div class="d-flex gap-2 mt-3 flex-wrap">
                                            <?php if (!empty($convite['wa_link_como_funciona'])): ?>
                                                <a href="<?= $e($convite['wa_link_como_funciona']) ?>" target="_blank" rel="noopener" class="btn btn-outline-success btn-sm">
                                                    <i class="fab fa-whatsapp me-1"></i>Abrir explicação
                                                </a>
                                            <?php endif; ?>

                                            <form method="post" action="<?= $e($bp) ?>/admin/marketing/prospeccao/lead/enviado/<?= (int)($lead['id'] ?? 0) ?>">
                                                <input type="hidden" name="csrf_token" value="<?= $e($csrfToken ?? '') ?>">
                                                <input type="hidden" name="mensagem_texto" value="<?= $e($utf8fix($convite['texto_como_funciona'] ?? '')) ?>">
                                                <input type="hidden" name="wa_link" value="<?= $e($utf8fix((string)($convite['wa_link_como_funciona'] ?? ''))) ?>">
                                                <button type="submit" class="btn btn-outline-primary btn-sm">Enviar como funciona</button>
                                            </form>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <tbody>
                                <?php foreach ($historico as $item): ?>
                                    ...
                                    <tr>
                                        ...
                                        <td><?= $e($fix($item['acao'] ?? '')) ?></td>
                                        <td><?= $e($fix($item['titulo'] ?? '')) ?></td>
                                        <td>
