...
foreach (($cidades ?? []) as $c) {
    $cidadesPayload[] = [
        ...
        'zonas' => array_map(static function (array $zona): array {
            return [
                'id' => (int)($zona['id'] ?? 0),
                'code' => (string)($zona['code'] ?? ''),
                'name' => (string)($zona['name'] ?? ''),
                'status_expansao' => (string)($zona['status_expansao'] ?? ''),
                'ordem_expansao' => $zona['ordem_expansao'] !== null ? (int)$zona['ordem_expansao'] : null,
                'active' => !empty($zona['active']),
            ];
        }, $zonasPorCidade[(int)$c['id']] ?? []),
...
<?php endif; ?>
<?php if (!empty($_GET['excluida'])): ?>
<div class="alert alert-success" style="margin:0 24px 16px;"><i class="fas fa-trash-check me-2"></i>Cidade excluída com sucesso e dependências subordinadas limpas.</div>
<?php endif; ?>
<?php if (isset($_GET['erro']) && $_GET['erro'] === 'senha'): ?>
<div class="alert alert-danger" style="margin:0 24px 16px;"><i class="fas fa-lock me-2"></i>Senha de administrador inválida. A exclusão não foi concluída.</div>
<?php endif; ?>
<?php if (isset($_GET['erro']) && $_GET['erro'] === 'excluir'): ?>
<div class="alert alert-danger" style="margin:0 24px 16px;"><i class="fas fa-circle-exclamation me-2"></i>Não foi possível excluir a cidade neste momento. Tente novamente.</div>
<?php endif; ?>
...
(function () {
    ...
    function renderCidade(cidadeId) {
        ...
        html += '<div class="card mb-3"><div class="card-header"><i class="fas fa-map-location-dot me-2"></i>Zonas vinculadas</div><div class="card-body">';
        if (c.zonas && c.zonas.length) {
            html += '<div class="list-group list-group-flush">';
            c.zonas.forEach(function (zona) {
                html += '<div class="list-group-item px-0 d-flex justify-content-between align-items-start gap-3">';
                html += '<div><strong>' + escapeHtml(zona.name) + '</strong><div class="small text-muted">' + escapeHtml(zona.code) + '</div></div>';
                html += '<div class="text-end"><span class="badge ' + (zona.active ? 'text-bg-success' : 'text-bg-secondary') + '">';
                html += (zona.active ? 'Ativa' : 'Inativa') + '</span>';
                if (zona.status_expansao) {
                    html += '<div class="small text-muted mt-1">' + escapeHtml(zona.status_expansao) + '</div>';
                }
                html += '</div></div>';
            });
            html += '</div>';
        } else {
            html += '<div class="text-muted small">Nenhuma zona vinculada a esta cidade.</div>';
        }
        html += '</div></div>';
        ...
        html += '<div class="card mb-3 border-danger"><div class="card-header bg-danger text-white"><i class="fas fa-trash me-2"></i>Excluir cidade</div><div class="card-body">' +
            '<div class="alert alert-danger mb-3"><strong>Irreversível:</strong> esta ação remove a cidade e tudo que estiver subordinado a ela, incluindo zonas de precificação e regras vinculadas. ' +
            'Os registros operacionais não são apagados, mas a cidade será desvinculada deles. Use apenas se tiver certeza.</div>' +
            '<form method="post" action="' + CID_BP + '/admin/cidade/excluir" onsubmit="return confirm(\'Excluir esta cidade? Esta ação é irreversível.\');" class="row g-2">' +
            '<input type="hidden" name="csrf_token" value="' + escapeHtml(CID_CSRF) + '">' +
            '<input type="hidden" name="id" value="' + c.id + '">' +
            '<div class="col-md-4"><label class="form-label small">Senha do administrador</label><input type="password" class="form-control form-control-sm" name="senha_admin" autocomplete="current-password" required></div>' +
            '<div class="col-md-8"><label class="form-label small">Justificativa para o log</label><textarea class="form-control form-control-sm" name="justificativa" rows="2" required placeholder="Explique por que esta cidade precisa ser removida"></textarea></div>' +
            '<div class="col-12 d-flex justify-content-end"><button type="submit" class="btn btn-danger"><i class="fas fa-trash me-1"></i>Excluir cidade</button></div>' +
            '</form></div></div>';
