<?php
/**
 * Fragmento do painel de detalhe de um guincho — conteúdo do workspace de
 * /admin/guinchos (shell-ops), devolvido como HTML puro por
 * AdminController::guinchoDetalheFragmento() e injetado via fetch()+innerHTML
 ...
 * guinhodetalhe.php (mantida como arquivo à parte, hoje sem rota própria).
 *
 * @var array $guincho
 * @var array|null $usuario
 * @var array $pedidos
 * @var array $capacidades
 * @var array $compatPorServico
...
<header class="ops-order-header">
    <div>
        <button type="button" class="ops-back-link" data-action="gu-clear-selection">
            <i class="fas fa-arrow-left"></i> Todos os guinchos
        </button>
        <h1>
            <?php echo htmlspecialchars($usuario['nome'] ?? 'Guincho'); ?>
            <?php if (!$guincho['aprovado']): ?>
            <span class="ops-badge ops-badge--new" style="margin-left:8px;">Pendente aprovação</span>
            <?php endif; ?>
        </h1>
        <p>Placa: <?php echo htmlspecialchars($guincho['placa_guincho'] ?? '—'); ?></p>
    </div>
    <div class="d-flex gap-2 flex-wrap">
        <a href="<?php echo htmlspecialchars($bp); ?>/admin/usuario/editar/<?php echo (int)($usuario['id'] ?? 0); ?>" class="ops-btn">
            <i class="fas fa-edit"></i> Editar
        </a>
        <a href="<?php echo htmlspecialchars($bp); ?>/admin/usuario/<?php echo (int)($usuario['id'] ?? 0); ?>" class="ops-btn">
            <i class="fas fa-user"></i> Ver usuário
        </a>
        <a href="<?php echo htmlspecialchars($bp); ?>/admin/avaliacoes?guincho_id=<?php echo (int)($guincho['id'] ?? 0); ?>" class="ops-btn">
    ...
    <div class="col-md-5">
            ...
            <div class="card-body">
                ...
                <div class="d-flex gap-2">
                    <form method="POST" action="<?php echo htmlspecialchars($bp); ?>/admin/guincho/aprovar" class="flex-fill">
                        ...
                        <input type="hidden" name="id" value="<?php echo (int)$guincho['id']; ?>">
        ...
        <div class="card mt-4">
            ...
            <div class="card-body d-flex flex-column gap-2">
                ...
                <a href="<?php echo htmlspecialchars($bp); ?>/arquivo/<?php echo (int)$guincho['id']; ?>?tipo=doc_cnh_frente" target="_blank" class="btn btn-outline-primary btn-sm">
                    <i class="fas fa-id-card me-1"></i>CNH Frente
                </a>
                <?php endif; ?>
                <?php if (!empty($guincho['doc_cnh_verso'])): ?>
                <a href="<?php echo htmlspecialchars($bp); ?>/arquivo/<?php echo (int)$guincho['id']; ?>?tipo=doc_cnh_verso" target="_blank" class="btn btn-outline-primary btn-sm">
                    <i class="fas fa-id-card me-1"></i>CNH Verso
                </a>
                <?php endif; ?>
                <?php if (!empty($guincho['foto_veiculo'])): ?>
                <a href="<?php echo htmlspecialchars($bp); ?>/arquivo/<?php echo (int)$guincho['id']; ?>?tipo=foto_veiculo" target="_blank" class="btn btn-outline-primary btn-sm">
    ...
    <div class="col-md-7">
        ...
        <div class="card mb-4">
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <a href="<?php echo htmlspecialchars($bp); ?>/admin/catalogo-servicos/capacidades" class="btn btn-sm btn-primary">
            ...
            <div class="card-body">
                <p class="small text-muted mb-3">
                    ...
                    <a href="<?php echo htmlspecialchars($bp); ?>/admin/catalogo-servicos/capacidades">fila de capacidades</a>. O reboque é aprovado ao aprovar o prestador na fila de pendentes.
                </p>
                ...
                <div class="p-2 mb-2 rounded" style="border:1px solid var(--theme-border,#333)">
                    <div class="d-flex align-items-center justify-content-between">
                        ...
                        <?php echo $statusBadgeFn($c['approval_status'] ?? ''); ?>
                    </div>
                    ...
                    <div class="mt-2">
                        ...
                        <div class="small mb-1" style="padding-left:.5rem;border-left:2px solid var(--brand,#2fb34a)">
                            <?php echo htmlspecialchars($catLbl[$cv['vehicle_category']] ?? $cv['vehicle_category']); ?>
                            — <?php echo $statusBadgeFn($cv['approval_status'] ?? ''); ?>
                            <?php
                            $flags = [];
                    ...
                    <table class="table table-hover mb-0">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td><a href="<?php echo htmlspecialchars($bp); ?>/admin/pedido/<?php echo $p['id']; ?>" class="btn btn-sm btn-outline-primary"><i class="fas fa-eye"></i></a></td>
                            </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
