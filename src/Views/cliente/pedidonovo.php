...
<div class="alert alert-danger mb-3" role="alert">
    ...
    $msgs = [
        ...
        'destino_reboque'     => 'Para seguir com a cotação do reboque, informe agora para qual endereço o veículo deve ser levado.',
        ...
        'sem_cobertura'       => 'Recebemos sua localização e seguimos ampliando as alternativas de atendimento nessa região. Revise o endereço ou tente novamente em alguns instantes para encontrarmos a melhor opção disponível.',
    ];
...
<div class="socorro-shell" id="socorroShell">
    ...
    <form method="POST" action="<?php echo $bp; ?>/cliente/pedido/criar" id="formPedido" class="socorro-sheet" data-marketing-event="create_order">
        ...
        <input type="hidden" name="endereco_destino"  id="endereco_destino"><input type="hidden" name="oficina_parceira_provider_id" id="oficina_parceira_provider_id" value="">
        ...
        <div class="socorro-step d-none" data-step="confirmar">
            ...
            <div id="destinoBlock" class="d-none">
                ...
                <?php endif; ?>
                <div id="destFeedback" class="form-text text-muted mt-1">Selecione uma oficina, busque um endereço ou toque no mapa.</div><div class="alert alert-info small mt-2"><strong>Oficina parceira (opcional)</strong><select class="form-select form-select-sm mt-1" id="selectOficinaParceira"><option value="">Escolha uma oficina parceira…</option><?php foreach (($oficinasParceiras ?? []) as $op): ?><option value="<?= (int)$op["id"] ?>" data-lat="<?=htmlspecialchars((string)($op["latitude"]??""))?>" data-lng="<?=htmlspecialchars((string)($op["longitude"]??""))?>" data-endereco="<?=htmlspecialchars((string)($op["address"]?:($op["trade_name"]?:$op["legal_name"])))?>"><?=htmlspecialchars($op["trade_name"]?:$op["legal_name"])?></option><?php endforeach; ?></select><small>O endereço e as coordenadas da oficina serão usados como destino.</small></div>
            </div>
...
const oficinaParceiraSelect = document.getElementById('selectOficinaParceira');
if (oficinaParceiraSelect) oficinaParceiraSelect.addEventListener('change', function(){ const o=this.options[this.selectedIndex]; document.getElementById('oficina_parceira_provider_id').value=this.value||''; if(this.value && o.dataset.lat && o.dataset.lng){ setDest(parseFloat(o.dataset.lat),parseFloat(o.dataset.lng),o.dataset.endereco); } });
